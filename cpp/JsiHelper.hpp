...
#define BIND_METHOD(method) std::bind(&method, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4)
...
#define BIND_THIS_GETTER(cls, name) std::bind(&cls::name##_get, this, std::placeholders::_1)

#define BIND_THIS_SETTER(cls, name) std::bind(&cls::name##_set, this, std::placeholders::_1, std::placeholders::_2)

#define METHOD_INFO(cls, name, count) {#name, {BIND_THIS_METHOD(cls, name), count}}

#define GETTER_INFO(cls, name) {#name, BIND_THIS_GETTER(cls, name)}

#define DEFINE_METHOD(name) Value name##_method(Runtime& runtime, const Value& thisValue, const Value* arguments, size_t count)
